# Documentation Python CI - Tests — modèle renforcé (.tox-config, multi-environnements)

← [Pipelines](../pipelines.fr.md) | ← [Pipeline Tests (modèle simple)](./tests.fr.md)

> **Statut : conservé pour l'exemple, non utilisé en production.** Ce document décrit une approche plus
> élaborée que celle de [Tests (modèle simple)](./tests.fr.md) — appuyée sur `.tox-config` pour la gestion
> de plusieurs environnements — telle qu'expérimentée sur le projet `sds`. Elle a été abandonnée au profit
> du modèle plus simple (un environnement tox par job de matrice, `tox -e py${PYVER}`) documenté dans
> [Tests](./tests.fr.md) et repris comme modèle de référence dans
> [`shared/github-ci/workflow-run/python-ci-tests.yaml`](../../../shared/github-ci/workflow-run/python-ci-tests.yaml).
>
> **Point d'attention si vous vous en inspirez :** ce document utilise
> `ref: ${{ github.event.workflow_run.head_branch }}` au checkout — voir la section correspondante
> ci-dessous. Le modèle actuellement recommandé utilise `head_sha`, pas `head_branch` (voir
> [Deux architectures de pipeline](../../architecture/ci-models.fr.md#modèle-1--chaînage-événementiel-workflow_run)) ;
> `head_branch` pointe vers le dernier commit de la branche au moment de l'exécution, pas nécessairement
> celui qui a déclenché la chaîne — une source de décalage que `head_sha` évite. Conservé ici tel quel pour
> fidélité historique au document original.
