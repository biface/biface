# Python CI - Tests Documentation — reinforced model (.tox-config, multi-environment)

← [Pipelines](../pipelines.en.md) | ← [Tests pipeline (simple model)](./tests.en.md)

> **Status: kept as an example, not used in production.** This document describes a more elaborate
> approach than [Tests (simple model)](./tests.en.md) — built on `.tox-config` for managing several
> environments — as experimented on the `sds` project. It was abandoned in favor of the simpler model (one
> tox environment per matrix job, `tox -e py${PYVER}`) documented in [Tests](./tests.en.md) and reused as
...
>
> **If you're drawing on this document:** it uses `ref: ${{ github.event.workflow_run.head_branch }}` at
> checkout — see the corresponding section below. The currently recommended model uses `head_sha`, not
> `head_branch` (see
> [Two pipeline architectures](../../architecture/ci-models.en.md#model-1--event-driven-chaining-workflow_run)) —
> `head_branch` points to the branch's latest commit at execution time, not necessarily the one that
> triggered the chain, a drift that `head_sha` avoids. Kept here as-is for fidelity to the original
> document.
